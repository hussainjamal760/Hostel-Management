...
const menuItems = [
  ...
  { name: 'Create Room', href: '/manager/create-room', icon: 'add_box' },
  { name: 'Manage Rooms', href: '/manager/rooms', icon: 'bed' },
  { name: 'Students', href: '/manager/students', icon: 'group' },
  { name: 'Expenses', href: '/manager/expenses', icon: 'receipt_long' },
  { name: 'Payments', href: '/manager/payments', icon: 'payments' },
  { name: 'Reports', href: '/manager/reports', icon: 'analytics' },
  { name: 'Complaints', href: '/manager/complaints', icon: 'report_problem' }
];
      ...
      >
        ...
        <div className="px-6 mt-auto pt-6 border-t border-outline-variant/30">
          <div className="p-4 bg-surface-container rounded-xl border border-outline-variant text-center">
            <p className="font-label-md text-label-md text-primary font-bold tracking-widest uppercase">Manager Portal</p>
      ...
      <header className={`fixed top-0 right-0 w-full z-40 bg-surface/80 backdrop-blur-md flex justify-between items-center px-6 md:px-10 h-20 shadow-sm border-b border-outline-variant transition-all duration-300 ${
        ...
        <div className="flex items-center gap-6">
          <div className="hidden lg:flex flex-col items-end mr-2">
            <span className="font-label-md text-label-md text-primary">Skyview Residence</span>
            <span className="text-xs text-on-surface-variant">East Wing Lead</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="h-10 w-10 rounded-full overflow-hidden border-2 border-secondary-container ml-2 bg-surface-container flex justify-center items-center font-bold text-primary">
              {mounted && user?.name ? getInitials(user.name) : 'M'}
            </div>
          </div>
        </div>
      </header>
